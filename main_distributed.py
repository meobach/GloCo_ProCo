...
def train(net, data_loader,train_coreset, center_criterion,train_optimizer,epoch, args,list_temperature_tensor,w_augment,w_imb,w_center,weight_sum):
    ...
    for images,pos,targets in train_bar:
        ...
        #augment loss
        loss_augment,_,_ = net(im_1_feature, [im_2])
        #positive loss
        loss_pos,_,_=net(im_1_feature,[pos_1,pos_2,pos_3])
        #ImbMoco loss
        loss_pos_total=(1-w_augment)*(loss_pos) + w_augment*(loss_augment)
        if(use_center):
            #calc prototype loss
            ...
            loss_center= weight_sum*(loss_v1_1 + loss_v2_2 + loss_v1_2 + loss_v2_1 )
            #loss_v1_1=center_criterion(features_f1_1,class_temperature,use_class_temperature)
            loss = w_imb*loss_pos_total + w_center*loss_center
        else:
            loss=loss_pos_total
